...
#include <math.h>
#include <stdlib.h>
#include "photonic_rng.h"

// Module-level RNG state (initialized lazily)
static PhoRng g_photonic_rng = {{12345, 67890}};
...
void photonic_layer_forward(const PhotonicLayer *layer, const Complex *input, Complex *output) {
    ...
    for (size_t i = 0; i < dim; i++) {
        ...
        if (layer->phase_noise > 0.0) {
            double phi = pho_rng_normal(&g_photonic_rng, 0.0, layer->phase_noise);
